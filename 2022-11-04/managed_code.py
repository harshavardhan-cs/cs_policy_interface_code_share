...
from cs_policy_interface.utils import AccessNestedDict, rem_duplicates_from_op
from cs_policy_interface.utils import get_mongo_client


THIRTY_MIN_TIMEOUT_LIMIT = 1800
...
class ManagedCode(object):
            ...
            for region in regions:
                try:
                    redshift_clusters = run_aws_operation(
                        credentials, 'redshift', 'describe_clusters', region_name=region, response_key='Clusters')
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
                for cluster_info in redshift_clusters:
                    evaluated_resources += 1
    ...
    def aws_cloudtrail_bucket_publicly_accessible(self, **kwargs):
        ...
        evaluated_resources = 0
        operation_args = {}
        permission_checking_list = ['READ', 'WRITE', 'READ_ACP', 'WRITE_ACP', 'FULL_CONTROL']
        grant_uris = ["http://acs.amazonaws.com/groups/global/AuthenticatedUsers",
        ...
        try:
            ...
            for region in regions:
                ...
                for cluster_name in cluster_info:
                    for clstr_param_grp in cluster_name.get('ClusterParameterGroups', []):
                        if not clstr_param_grp.get('ParameterGroupName'):
                            continue

                        cluster_response = run_aws_operation(
                            credentials, 'redshift', 'describe_cluster_parameters',
                            operation_args={'ParameterGroupName': clstr_param_grp['ParameterGroupName']},
                            response_key='Parameters',
                            region_name=region)
                        evaluated_resources += 1
                        for parameter_info in cluster_response:
                            if parameter_info.get('ParameterName') == "require_ssl" and parameter_info.get(
                                ...
                                output.append(
                                    OrderedDict(
                                        ...
                                        ResourceName=cluster_info.get('ClusterIdentifier'),
                                        Resource='Cluster'))
            output = rem_duplicates_from_op(output)
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))
                ...
                try:
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                for policy in default_policy_response['PolicyDescriptions']:
                    default_policy_list.append(policy['PolicyName'])
                for elb in elb_response:
                    evaluated_resources += 1
                    for policyname in elb['ListenerDescriptions']:
                        if policyname['Listener']['Protocol'] == 'HTTPS':
                            if policyname['PolicyNames'] not in default_policy_list:
                                output.append(
                                    OrderedDict(
    ...
    def aws_ec2_managed_instance_applications_blacklisted(self, **kwargs):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
                    ...
                    for instance in reservation.get('Instances'):
                        ...
                        try:
                            ...
                            raise Exception(
                                'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                                    str(e)))
                        if not ssm_response.get('Entries'):
                            output.append(
                                OrderedDict(
            ...
            for region in regions:
                try:
                    volume_response = run_aws_operation(
                        credentials, 'ec2', 'describe_volumes', response_key='Volumes', region_name=region)
                    for ebs_volume_info in volume_response:
                        evaluated_resources += 1
                        if ebs_volume_info.get('State') == 'available':
                            output.append(
                                OrderedDict(
                                    ResourceId=ebs_volume_info.get('VolumeId', ''),
                                    ResourceName=ebs_volume_info.get('VolumeId', ''),
                ...
                for table in db_response:
                    ...
                    try:
                        backup_response = run_aws_operation(
                            ...
                            region_name=region,
                            operation_args=operation_args)
                        if backup_response.get('ContinuousBackupsDescription', {}).get(
        ...
        try:
            ...
            for region in regions:
                try:
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(str(e)))
                for domains in es_domain_response.get('DomainNames', []):
                    evaluated_resources += 1
        ...
        try:
            ...
            for iam in iam_response:
                ...
                iam_user_response = run_aws_operation(
                    credentials, 'iam', 'list_access_keys', operation_args)
                for access_key_usage in iam_user_response.get('AccessKeyMetadata', []):
                    if access_key_usage.get('Status') == 'Active':
                        output.append(
                            OrderedDict(
        ...
        try:
            ...
            for s3_bucket in s3_buckets['Buckets']:
                ...
                try:
                    ...
                    if s3_bucket_website.get('IndexDocument', {}).get('Suffix') == '':
                        output.append(
                            OrderedDict(
                                ResourceId=s3_bucket['Name'],
                                ResourceName=s3_bucket['Name'],
        ...
        try:
            ...
            for roles in iam_response:
                ...
                for statement in iam_role_response.get('Role', {}).get('AssumeRolePolicyDocument', {}).get(
                        'Statement', []):
                    if statement.get('Condition', {}).get('Bool', {}).get('aws:MultiFactorAuthPresent'):
                        output.append(
                            OrderedDict(
            ...
            for region in regions:
                ...
                for policy in fms_response:
                    ...
                    fms_policy_response = run_aws_operation(
                        ...
                        region_name=region,
                        operation_args=operation_args)
                    for policy_info in fms_policy_response['Policy']:
                        policy_data = json.loads(
                            policy_info['SecurityServicePolicyData']['ManagedServiceData'])
            ...
            for region in regions:
                ...
                for policy in fms_response:
                    ...
                    fms_policy_response = run_aws_operation(
                        ...
                        operation_args=operation_args)
                    for policy_info in fms_policy_response['Policy']:
                        if policy_info['SecurityServicePolicyData']['Type'] != 'WAF' or policy_info[
                ...
                for key in kms_response:
                    ...
                    try:
                        ...
                        if 'No such policy exists' in str(e):
                            continue
                        raise e
                    kms_policy = json.loads(key_info.get('Policy', '{}'))
                    for principal in kms_policy.get('Statement', []):
                        if principal.get('Principal', {}).get('AWS') == "*":
                            output.append(
                                OrderedDict(
            ...
            for region in regions:
                ...
                for ec2_reservations in ec2_instance_response:
                    for ec2_instance_info in ec2_reservations['Instances']:
                        evaluated_resources += 1
                        if not ec2_instance_info.get('IamInstanceProfile'):
                            output.append(
                                OrderedDict(
            ...
            for region in regions:
                try:
                    dms_response = run_aws_operation(
                        ...
                        region_name=region,
                        response_key='ReplicationInstances')
                except Exception as e:
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                                    ...
                                    for tag in tags['Tags']:
                                        if check_type == 'app_tier_tag':
                                            ...
                                            balance_response = run_aws_operation(
                                                credentials, 'elb', 'describe_load_balancer_policies',
                                                operation_args, region_name=region)
                                            for policy in balance_response['PolicyDescriptions']:
                                                if any(desc['AttributeName'] == 'Reference-Security-Policy' for
                                                       desc in policy['PolicyAttributeDescriptions']) and \
                                                        policy['PolicyName'] in POLICY_NAME_LIST:
                                                    # NON_COMPLIANT
                                                    output.append(
                                                        OrderedDict(
    ...
    def common_ssl_tls_server(self, credentials, region, output):
        try:
            response = run_aws_operation(
                credentials,
                'elb',
                'describe_load_balancers',
                region_name=region)
            for listener_description in response['LoadBalancerDescriptions']:
                for instance_port in listener_description['ListenerDescriptions']:
                    instance_port_value = instance_port['Listener']['InstancePort']

                    if listener_description['ListenerDescriptions'] != "":
                        output.append(
                            OrderedDict(
                                ResourceId=instance_port_value,
                                ResourceName=instance_port_value,
                                ResourceType='ELB'))
            return output
        except Exception as e:
            raise Exception(str(e))
    ...
    def aws_cloudtrail_bucket_mfa_delete_enabled(self, **kwargs):
        ...
        try:
            ...
            for region in regions:
                credentials = self.execution_args['auth_values']
                cloudtrail_response = run_aws_operation(
                    credentials,
                    'cloudtrail',
                    'describe_trails',
                    region_name=region)
                for trail in cloudtrail_response['trailList']:
                    evaluated_resources += 1
                    operation_args.update(Bucket=trail['S3BucketName'])
                    versioning = run_aws_operation(
            ...
            for region in regions:
                ...
                for elb in elb_response:
                    ...
                    elb_descriptions = run_aws_operation(
                        ...
                        operation_args=operation_args,
                        region_name=region)
                    if not (elb_descriptions.get('LoadBalancerAttributes', {}).get('CrossZoneLoadBalancing',
                                                                                   {}).get('Enabled') and len(
                        elb['AvailabilityZones']) > len(elb['Instances'])):
                        output.append(
                            OrderedDict(
                                ResourceId=elb['LoadBalancerName'],
                                ResourceName=elb['LoadBalancerName'],
            ...
            for region in regions:
                try:
                    ...
                    asg_response = run_aws_operation(
                        ...
                        region_name=region,
                        response_key='AutoScalingInstances')
                    for reservation in ec2_instance_response:
                        for instance in reservation['Instances']:
                            instanceidslist.append(instance['InstanceId'])
                            evaluated_resources += 1
                    for asg in asg_response:
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                auto_scaling_instance = set(auto_scaling_instance_list)
                result = [x for x in instanceidslist if x not in auto_scaling_instance]
        ...
        try:
            ...
            for region in regions:
                if check_type == 'Snapshot_encrypted':
                    ...
                    try:
                        operation_args = {}
                        operation_args.update(OwnerIds=[service_account_id])
            ...
            for region in regions:
                try:
                    rds_cluster_info = run_aws_operation(
                        ...
                        region_name=region,
                        response_key='DBClusters')
                    rds_instance = run_aws_operation(
                        credentials,
                        'rds',
                        'describe_db_instances',
                        region_name=region,
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                for sg in rds_cluster_info:
                    evaluated_resources += 1
                    if sg.get('VpcSecurityGroups'):
                        for sg_cluster_id in sg.get('VpcSecurityGroups', []):
                            operation_args.update(Filters=[
                                {
                                    'Name': 'group-id',
                                    'Values': [
                                        sg_cluster_id.get('VpcSecurityGroupId')
                                    ]
                                }
                            ])
                            cluster_security_groups = run_aws_operation(
                                credentials,
                                'ec2',
                                'describe_security_groups',
                                region_name=region,
                                operation_args=operation_args)
                            for security_group in cluster_security_groups.get('SecurityGroups', []):
                                for inpermission in security_group.get('IpPermissions', []):
                                    for ip_address in inpermission.get('IpRanges', []):
                                        if ip_address.get('CidrIp') == '0.0.0.0/0':
                                            output.append(
                                                OrderedDict(
                                                    ResourceId=sg['DBClusterIdentifier'],
                                                    ResourceName=sg['DBClusterIdentifier'],
                                                    Resource='rds'))
                for sg_info in rds_instance:
                    evaluated_resources += 1
                    if sg_info.get('VpcSecurityGroups'):
                        for sg_id in sg_info.get('VpcSecurityGroups', []):
                            operation_args.update(Filters=[
                                {
                                    'Name': 'group-id',
                                    'Values': [
                                ...
                                for inpermission in security_group.get('IpPermissions', []):
                                    for ip_address in inpermission.get('IpRanges', []):
                                        if ip_address.get('CidrIp') == '0.0.0.0/0':
                                            output.append(
                                                OrderedDict(
                                                    ResourceId=sg_info['DBInstanceIdentifier'],
                                                    ResourceName=sg_info['DBInstanceIdentifier'],
                                                    Resource='rds'))
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))
    ...
    def aws_app_tier_kms_customer_master_key_in_use(self):
        app_tier_tag_key = self.execution_args["args"]["app_tier_tag_key"]
        app_tier_tag_value = self.execution_args["args"]["app_tier_tag_value"]
        output, evaluated_resources = self.common_kms_customer_master_key_in_use(app_tier_tag_key, app_tier_tag_value)
        return output, evaluated_resources
    ...
    def aws_key_app_tier_kms_customer_master_key_in_use(self, **kwargs):
        ...
        try:
            credentials = self.execution_args['auth_values']
            app_tier_tag = self.execution_args['args']["app_tier_tag"]
            app_tier_tag_value = self.execution_args['args']["app_tier_tag_value"]
            regions = [region.get('id')
                       for region in self.execution_args['regions']]
            for region in regions:
        ...
        try:
            ...
            for roles in iam_response:
                ...
                for statement in iam_role_response.get('Role', {}).get('AssumeRolePolicyDocument', {}).get(
                        'Statement', []):
                    if 'AWS' in statement.get('Principal', ''):
                        if isinstance(statement.get('Principal', {}).get('AWS', ':::'), str):
                            aws_account_id = statement.get('Principal', {}).get('AWS', ':::').split(':')[-2]
                            if aws_account_id != account_id:
                                output.append(
                                    OrderedDict(
    ...
    def aws_ebs_web_tier_encrpted(self):
        output, evaluated_resources = self.common_ebs_encrpted()
        return output, evaluated_resources
            ...
            for region in regions:
                ...
                for each_dbcluster in db_clusters:
                    ...
                    if not each_dbcluster.get("DeletionProtection"):
                        output.append(
                            OrderedDict(
                                ResourceId=each_dbcluster['DatabaseName'],
                                ResourceName=each_dbcluster['DatabaseName'],
                                Resource='RDS'))
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))
        ...
        try:
            ...
            for region in regions:
                ...
                for info in rds_response:
                    evaluated_resources += 1
                    if config not in info or not info[config]:
                        output.append(
                            OrderedDict(
    ...
    def aws_rds_security_groups_events_subscriptions(self, **kwargs):
        ...
        try:
            ...
            for region in regions:
                ...
                evaluated_resources += 1
                for info in rds_response:
                    if info.get('SourceType') == 'db-security-group':
        ...
        try:
            ...
            try:
                content = run_aws_operation(
                    credentials, 'iam', 'get_credential_report').get('Content', '{}').decode()
            except Exception as err:
                if format(err.response['Error']['Code']) == 'ReportNotPresent':
                    generate_updated_report = run_aws_operation(
                        credentials, 'iam', 'generate_credential_report')
                    content = run_aws_operation(
                        credentials, 'iam', 'get_credential_report').get('Content', '{}').decode()
                else:
                    raise err
                ...
                try:
                    if check_type == 'access_key_active':
                        ...
                        if data[0] == '<root_account>':
                            password_last_used = data[4].split('+')[0]
                            date_object = datetime.strptime(password_last_used, "%Y-%m-%dT%H:%M:%S")
                            now = datetime.now()
                            if (now - date_object).days <= 7:
                                output.append(
                                    OrderedDict(
                                        ResourceId=data[0],
                                        ResourceName=data[0],
                        ...
                        evaluated_resources = 1
                        if data[0] == '<root_account>':
                            if (data[18] == 'false') or (data[20] == 'false'):
                                # Non-Compliant
                                output.append(
                                    OrderedDict(
                                        ResourceId=data[0],
                        ...
                        user_creation_time = user_creation_time_index.split(
                            '+')[0]
                        if user_creation_time:
                            user_creation_time = DT.strptime(
                                user_creation_time, "%Y-%m-%dT%H:%M:%S").date()
                        access_key_1_last_rotated = access_key_1_last_rotated_index.split(
                            '+')[0]
                        if access_key_1_last_rotated:
                            access_key_1_last_rotated = DT.strptime(
                                access_key_1_last_rotated, "%Y-%m-%dT%H:%M:%S").date()
                        ...
                        for user in user_response:
                            ...
                            if len(attached_user) != 0:
                                if data[0] == user['UserName']:
                                    if not (data[3] == 'false' and data[4] == 'N/A'):
                                        if data[8] != 'true' or data[13] != 'true':
                                            # Non Complaint
                                            output.append(
                                                OrderedDict(
            ...
            for region in regions:
                try:
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                for dbinstance in rds_response['DBInstances']:
                    evaluated_resources += 1
                    for subnets in dbinstance.get('DBSubnetGroup', {}).get('Subnets'):
                        operation_args.update(Filters=[
                            {'Name': 'association.subnet-id', 'Values': [subnets['SubnetIdentifier'], ]}, ])
                        routeresponse = run_aws_operation(
                            credentials,
                            'ec2',
                            'describe_route_tables',
                            operation_args=operation_args,
                            region_name=region)
                        for route in routeresponse['RouteTables']:
                            for ing in route['Routes']:
                                try:
                                    if ing['GatewayId'] and ing['DestinationCidrBlock'] == '0.0.0.0/0':
                                        output.append(
                                            OrderedDict(
                                                ResourceId=dbinstance['DBInstanceIdentifier'],
                                                ResourceName=dbinstance['DBInstanceIdentifier'],
    ...
    def aws_web_tier_elb_security_policy(self):
        output, evaluated_resources = self.common_ssl_tls_server_certificate_to_tier(
            'security_policy')
        return output, evaluated_resources
    ...
    def aws_rds_desired_instance_type(self):
        try:
            output = list()
            evaluated_resources = 0
            ...
            for region in regions:
                ...
                for _data in db_clusters:
                    evaluated_resources += 1
                    if _data.get('DBInstanceClass') not in desired_instance_list:
                        output.append(
                            OrderedDict(
        ...
        try:
            ...
            for region in regions:
                ...
                for domains in es_response['DomainNames']:
                    ...
                    value = esresponse.get('DomainStatus', {}).get('AccessPolicies')
    ...
    def aws_audit_s3_bucket_open_to_the_world_that_web_server(self, **kwargs):
        output = list()
        evaluated_resources = 0
        try:
            credentials = self.execution_args['auth_values']
        ...
        try:
            ...
            for bucket in s3_buckets['Buckets']:
                ...
                try:
                    ...
                    for statement in policy['Statement']:
                        aws_account_id = statement.get('Principal', {}).get('AWS').split(':')[-2]
                        if aws_account_id not in account_access_list:
                            output.append(
                                OrderedDict(
                                    ResourceId=bucket['Name'],
                                    ResourceName=bucket['Name'],
            ...
            for region in regions:
                ...
                for db_instance in rds_response:
                    ...
                    for db_parameter in db_instance['DBParameterGroups']:
                        ...
                        db_parameter_response = run_aws_operation(
                            credentials,
                            'rds',
                            ...
                            operation_args=operation_args,
                            response_key='Parameters')
                        for parameter in db_parameter_response:
    ...
    def common_aws_rds_reserved_db_instance_expiration(self, expiry_days):
        try:
            output = list()
            evaluated_resources = 0
            now = datetime.now()
            ...
            for region in regions:
                ...
                for each_rd_instance in db_clusters:
                    ...
                    ri_start_time = each_rd_instance["StartTime"].replace(
                        tzinfo=None)
                    days = each_rd_instance['Duration'] // seconds_in_day
                    expire_time = ri_start_time + timedelta(days)
                    print((expire_time - now).days)
                    if (expire_time - now).days <= expiry_days:
                        output.append(
                            OrderedDict(
                                DBName=each_rd_instance['ReservedDBInstanceId'],
                                DBType=each_rd_instance['ReservedDBInstanceId'],
                                ResourceType='rds'))
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))

    def aws_rds_reserved_db_instance_expiration_in_30_days(self):
        output, evaluated_resources = self.common_aws_rds_reserved_db_instance_expiration(30)
        return output, evaluated_resources

    def aws_rds_reserved_db_instance_expiration_in_7_days(self):
        output, evaluated_resources = self.common_aws_rds_reserved_db_instance_expiration(7)
        return output, evaluated_resources
    ...
    def aws_ami_naming_convention(self, **kwargs):
        ...
        operation_args = {}
        evaluated_resources = 0
        environment = self.execution_args['args'].get("tag_naming_convention", "")
        ...
        try:
            ...
            regions = [region.get('id')
                       for region in self.execution_args['regions']]
            pattern = re.compile("^ami-(" + "|".join(regions) + ")-(" + "|".join(environment) + ")-([a-z0-9\-]+)$")
            for region in regions:
                try:
                    operation_args.update(Owners=['self', ])
                    ec2_response = run_aws_operation(
    ...
    def verify_that_your_lightsail_buckets_are_not_publicly_accessible(self, **kwargs):
        ...
        try:
            credentials = self.execution_args['auth_values']
            regions = [region.get('id') for region in self.execution_args['regions']]
            accessible_regions = self.get_lightsail_accessible_regions(regions)
    ...
    def get_lightsail_accessible_regions(self, regions_list):
        try:
            response = run_aws_operation(self.execution_args['auth_values'], 'lightsail', 'get_regions',
                                         region_name='us-east-1')
            supported_regions = [r['name'] for r in response['regions']]
            accessible_regions_per_service = [region for region in regions_list if region in supported_regions]
            return accessible_regions_per_service
        except Exception as e:
            raise Exception(str(e))
    ...
    def aws_cw_metrics_enabled(self, **kwargs):
        ...
        try:
            credentials = self.execution_args['auth_values']
            list_metrics = self.execution_args['args'].get('list_metrics').split(',')
            name_space = self.execution_args['args']['name_space']
            name_space = str(name_space).strip()
            regions = [region.get('id')
                       for region in self.execution_args['regions']]
            for region in regions:
                try:
                    for cloudwatch_metric in list_metrics:
                        evaluated_resources += 1
                        cloudwatch_metric = str(cloudwatch_metric).strip()
                        cloudwatch_response = run_aws_operation(
                            credentials,
                            'cloudwatch',
                            'describe_alarms_for_metric',
            ...
            for region in regions:
                try:
                    ...
                    raise Exception(
                        'Permission Denied or Region is not enabled to access resource. Error {}'.format(
                            str(e)))
                for db_info in db_clusters:
                    evaluated_resources += 1
                    if not db_info['MultiAZ']:
                        output.append(
                            OrderedDict(
                                ResourceId=db_info['DBClusterIdentifier'],
                                ResourceName=db_info['DBClusterIdentifier'],
        ...
        try:
            ...
            for region in regions:
                ...
                for elb in elb_response['LoadBalancerDescriptions']:
                    evaluated_resources += 1
                    operation_args.update(LoadBalancerNames=[elb['LoadBalancerName']])
                    tag_info = run_aws_operation(
                        credentials,
                        'elb',
                        'describe_tags',
    ...
    def aws_ec2_securitygroup_with_bastionssh(self, **kwargs):
        ...
        try:
            credentials = self.execution_args['auth_values']
            allowed_host = list(map(str.strip, self.execution_args['args']['allowed_host'].split(',')))
            regions = [region.get('id')
                       for region in self.execution_args['regions']]
            for region in regions:
                try:
    ...
    def aws_elasticsearch_minimum_version(self, **kwargs):
        ...
        try:
            credentials = self.execution_args['auth_values']
            elasticsearch_minimum_version = self.execution_args['elasticsearch_minimum_version']
            domain_names = list()
            regions = [region.get('id')
                       for region in self.execution_args['regions']]
            for region in regions:
                ...
                operation_args = {'DomainNames': domain_names}
                esresponse = run_aws_operation(
                    credentials,
                    'es',
    ...
    def aws_shield_srt_has_direct_access(self):
        output = list()
        evaluated_resources = 0
        boto_shield_obj = boto3.client('shield')
        ...
        try:
            ...
            for region in [region.get('id') for region in self.execution_args['regions']]:
                evaluated_resources += 1
                try:
                    drt_access = run_aws_operation(
                        credentials,
                        'shield',
                        'describe_drt_access',
                        region_name=region
                    )
                except boto_shield_obj.exceptions.ResourceNotFoundException:
                    continue
                except Exception as e:
                    raise Exception(str(e))

                if not drt_access.get('RoleArn'):
                    output.append(OrderedDict(
                        ResourceId=self.execution_args['service_account_id'],
                        ResourceName=self.execution_args['service_account_id'],
                        ResourceType='Shield'
                    ))
    ...
    def aws_cloudfront_viewer_policy_comm_https(self):
        output = list()
        evaluated_resources = 0
        ...
        try:
            credentials = self.execution_args['auth_values']
            for region in [region.get('id') for region in self.execution_args['regions']]:
                if (datetime.utcnow() - start_time).total_seconds() >= THIRTY_MIN_TIMEOUT_LIMIT:
                    break
                cloud_front_response = run_aws_operation(
                    credentials, 'cloudfront', 'list_distributions', region_name=region)
                evaluated_resources += 1
                for cloud_front_info in cloud_front_response.get('DistributionList', {}).get('Items', []):
                    if cloud_front_info.get('DefaultCacheBehavior', {}).get('ViewerProtocolPolicy') == 'allow-all':
                        output.append(OrderedDict(
                            ResourceId=cloud_front_info.get('Id'),
                            ResourceName=cloud_front_info['ARN'],
                            ResourceType='Stack'
                        ))
                    else:
                        for cb_item in cloud_front_info.get('CacheBehaviors', {}).get('Items', []) == 'allow-all':
                            if cb_item.get('ViewerProtocolPolicy') == 'allow-all':
                                output.append(OrderedDict(
                                    ResourceId=cloud_front_info.get('Id'),
                                    ResourceName=cb_item.get('TargetOriginId'),
                                    ResourceType='Stack'
                                ))
    ...
    def aws_ec2_ebs_optimized_instance(self, **kwargs):
        output = list()
        evaluated_resources = 0
        start_time = datetime.utcnow()
        try:
            ...
            regions = [region.get('id') for region in self.execution_args['regions']]
            for region in regions:
                instance_list = run_aws_operation(
                    credentials, 'ec2', 'describe_instances', region_name=region, response_key='Reservations')
                evaluated_resources += 1

                for instance in instance_list:
                    if not instance.get('EbsOptimized'):
                        output.append(OrderedDict(
                            ResourceId=instance.get('InstanceId'),
                            ResourceName=instance.get('InstanceId'),
                            ResourceType='EC2'
    ...
    def aws_api_gw_cache_enabled_and_encrypted(self):
        output = list()
        evaluated_resources = 0
        start_time = datetime.utcnow()
        try:
            ...
            for region in [region.get('id') for region in self.execution_args['regions']]:
                ...
                for api in rest_apis:
                    ...
                    for item in stages.get('item', []):
                        if not (item.get('methodSettings', {}).get('string', {}).get('cachingEnabled') and
                                item.get('methodSettings', {}).get('string', {}).get('cacheDataEncrypted')):
                            output.append(
                                OrderedDict(
    ...
    def aws_s3_transfer_acceleration(self):
        output = list()
        evaluated_resources = 0
        start_time = datetime.utcnow()
        try:
            credentials = self.execution_args['auth_values']
            for region in [region.get('id') for region in self.execution_args['regions']]:
                s3_buckets = run_aws_operation(credentials, 's3', 'list_buckets', region_name=region)

                for s3_bucket in s3_buckets.get('Buckets', []):
                    s3_bucket_acc = run_aws_operation(credentials, 's3', 'get_bucket_accelerate_configuration',
                                                      region_name=region,
                                                      operation_args={'Bucket': s3_bucket.get('Name')})
                    evaluated_resources += 1
                    if s3_bucket_acc.get('Status') != 'Enabled':
                        output.append(
                            OrderedDict(
                                ResourceId=s3_bucket.get('Name'),
                                ResourceName=s3_bucket.get('Name'),
    ...
    def discovery_check(self, service_account_id):
        try:
            ...
            query_to_check = {"service_account_id": service_account_id, "is_deleted": False,
                              ...
                              "data_sync.message": "Completed buckets sync"}
            account_info = list()
        ...
        try:
            ...
            with googleapiclient.discovery.build('cloudresourcemanager', 'v1',
                ...
                try:
                    ...
                    while True:
                        roles = ["roles/owner", "roles/writer", "roles/reader", "roles/viewer", "roles/editor"]
                        for members in request.get('bindings', []):
                            evaluated_resources += 1
                            if members.get("role", "") in roles:
        ...
        try:
            ...
            for region in regions:
                ...
                for key in kms_response:
                    if tier_key:
                        break
                    if 'alias/aws/' not in key['AliasName']:
                        try:
                            ...
                            key_response = run_aws_operation(
                                ...
                                region_name=region,
                                operation_args=operation_args)
                            if key_response['Tags']:
                                for tag in key_response['Tags']:
                                    if tag['TagKey'] == tag_key and tag['TagValue'] == tag_value:
                                        tier_key = True
                                        break
                        except Exception as e:
                            if 'TargetKeyId' in str(e):
        ...
        try:
            ...
            for region in regions:
                ...
                for vpc_endpoint in vpc_endpoints:
                    ...
                    vpc_endpoint_policy_document = json.loads(vpc_endpoint.get('PolicyDocument', '{}'))
                    vpc_endpoint_policy_document = vpc_endpoint_policy_document.get('Statement', [])
                    if not vpc_endpoint_policy_document or not isinstance(vpc_endpoint_policy_document, list):
                        continue
                    for attribute in vpc_endpoint_policy_document:
                        if attribute.get('Condition', {}).get('Bool', {}).get('aws:SecureTransport') == "false" and \
                                attribute.get('Effect') != 'Deny':
                            vpc_endpoint_has_secure_transport = True
                            break
                    if not vpc_endpoint_has_secure_transport:
                        output.append(
                            OrderedDict(
                                ResourceId=vpc_endpoint.get('VpcEndpointId'),
                                ResourceName=vpc_endpoint.get('VpcEndpointId'),
                                Resource='VPC'))
            return output, evaluated_resources
        except Exception as e:
            raise Exception(str(e))
    ...
    def aws_lambda_with_secure_environment_variables(self, **kwargs):
        ...
        evaluated_resources = 0
        operation_args = {}
        words = r'(?:key|secret|pass|pw|credential|token)(?!.*?(?:id|context|endpoint|url))'
        credentials = self.execution_args['auth_values']
        operation_args.update(FunctionVersion='ALL')
        try:
            regions = [region.get('id')
            ...
            for region in regions:
                ...
                for function in lambda_response:
                    ...
                    try:
                        sensitive_variables = []
                        insecure_variables = []
                        function_info = run_aws_operation(
                            credentials,
                            'lambda',
                            'get_function_configuration',
                            region_name=region,
                            operation_args=operation_args)
                        for name, value in function_info['Environment']['Variables'].items():
                            if re.search(words, name.lower()):
                            ...
                            if not port_range.get('From'):
                                output.append(
                                    OrderedDict(
                                        ...
                                        ResourceName=network_acl['NetworkAclId'],
                                        Resource='NACL'))
                            if port_range.get('From', '') not in security_group_ports:
                                output.append(
                                    OrderedDict(
                                        ResourceId=network_acl['NetworkAclId'],
                                        ResourceName=network_acl['NetworkAclId'],
    ...
    def common_publicly_shared_ami(self):
        output = list()
        operation_args = {}
        evaluated_resources = 0
        try:
    ...
    def aws_app_tier_publicly_shared_ami(self):
    ...
    def azure_storage_disk_snapshots_orphaned(self):
        try:
            ...
            service_account_id = self.execution_args["service_account_id"]
            inventory_db = get_mongo_client(self.connection_args)['resource_inventory']
            inventory_query = {"service_account_id": service_account_id,
                               "resource_type": "Storage_Disks",
                               "resource": "Snapshot",
                               "is_deleted": False}
            resource_list = inventory_db["service_resource_inventory"].find(inventory_query)
            for inventory in resource_list:
    ...
    def azure_mysql_db_servers_orphaned(self):
        try:
            output = list()
            evaluated_resources = 0
            service_account_id = self.execution_args["service_account_id"]
            inventory_db = get_mongo_client(self.connection_args)['resource_inventory']
            database_query = {"service_account_id": service_account_id,
                              "resource_type": "Databases",
                              "resource": "MySQL_Databases",
                              "is_deleted": False,
                              "$nor": [{"check_resource_element": "information_schema"},
                                       {"check_resource_element": "mysql"},
                                       {"check_resource_element": "performance_schema"},
                                       {"check_resource_element": "sys"}]
                              }
            db_parent_list = inventory_db["service_resource_inventory"].distinct(
                "additional_attributes.check_resource_parent_id", database_query)
            orphaned_sql_servers = inventory_db["service_resource_inventory"].aggregate([{"$match": {
                "service_account_id": service_account_id,
                "resource_type": "Servers",
                "resource": "MySQL",
                "is_deleted": False,
                "check_resource_element": {"$nin": db_parent_list}
            }},
                {"$project": {
                    "_id": 0,
                    "Resource": "$resource",
    ...
    def azure_pgsql_db_servers_orphaned(self):
        try:
            output = list()
            evaluated_resources = 0
            service_account_id = self.execution_args["service_account_id"]
            ...
            database_query = {"service_account_id": service_account_id,
                              "resource_type": "Databases",
                              "resource": "PGSQL_Databases",
                              "is_deleted": False,
                              "$nor": [{"check_resource_element": {"$regex": "databases/azure_sys"}},
                                       {"check_resource_element": {"$regex": "databases/azure_maintenance"}},
                                       {"check_resource_element": {"$regex": "databases/postgres"}}]
                              }
            db_parent_list = inventory_db["service_resource_inventory"].distinct(
                "additional_attributes.check_resource_parent_id", database_query)
            orphaned_sql_servers = inventory_db["service_resource_inventory"].aggregate([{"$match": {
    ...
    def azure_maria_db_servers_orphaned(self):
        try:
            output = list()
            evaluated_resources = 0
            service_account_id = self.execution_args["service_account_id"]
            ...
            database_query = {"service_account_id": service_account_id,
                              "resource_type": "Databases",
                              "resource": "MariaDB_Databases",
                              "is_deleted": False,
                              "$nor": [{"check_resource_element": {"$regex": "databases/aria_log_control"}},
                                       {"check_resource_element": {"$regex": "databases/performance_schema"}},
                                       {"check_resource_element": {"$regex": "databases/mysql"}},
                                       {"check_resource_element": {"$regex": "databases/information_schema"}}]
                              }
            db_parent_list = inventory_db["service_resource_inventory"].distinct(
                "additional_attributes.check_resource_parent_id", database_query)
            orphaned_sql_servers = inventory_db["service_resource_inventory"].aggregate([{"$match": {
