...
def evaluate_compliance(event):
    ...
    LOG.info(event)
    client = get_client('cloudtrail', event)
    watch_client = get_client('logs', event)
    cloud_watch_client = get_client('cloudwatch', event)
    sns = get_client('sns', event)
    evaluations = []
    try:
        response = client.describe_trails(trailNameList=[], includeShadowTrails=False)
        filter = '{ ($.eventName=DeleteGroupPolicy)||($.eventName=DeleteRolePolicy)||($.eventNa \
        ...
        for trail in response["trailList"]:
            if trail["IsMultiRegionTrail"]:
                ...
                if trail.get("CloudWatchLogsLogGroupArn"):
                    log_group = trail.get("CloudWatchLogsLogGroupArn").split(":")[6]
                    if status["IsLogging"]:
                        event_selectors = client.get_event_selectors(TrailName=trail["TrailARN"])
                        for event_val in event_selectors["EventSelectors"]:
                            if event_val["IncludeManagementEvents"] and event_val["ReadWriteType"] == "All":
                                metrics = watch_client.describe_metric_filters(logGroupName=log_group)
                                if metrics["metricFilters"]:
                                    for metric in metrics["metricFilters"]:
                                        if metric["filterPattern"] == filter:
                                            for metric_transformation in metric["metricTransformations"]:
                                                alarms = cloud_watch_client.describe_alarms_for_metric(
                                                    MetricName=metric_transformation["metricName"],
                                                    Namespace=metric_transformation["metricNamespace"])
                                                if alarms["MetricAlarms"]:
                                                    for alarm in alarms["MetricAlarms"]:
                                                        for topic_arn in alarm["AlarmActions"]:
                                                            topic_subscription = sns.list_subscriptions_by_topic(
                                                                TopicArn=topic_arn)
                                                            for owner in topic_subscription["Subscriptions"]:
                                                                if owner["SubscriptionArn"]:
                                                                    not_supported = False
                                                                    break
        if not_supported:
            evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event, annotation=''))
        else:
            evaluations.append(build_evaluation(event["accountId"], 'COMPLIANT', event, annotation=''))
        return evaluations
    except watch_client.exceptions.ResourceNotFoundException as e:
        evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event, annotation=''))
