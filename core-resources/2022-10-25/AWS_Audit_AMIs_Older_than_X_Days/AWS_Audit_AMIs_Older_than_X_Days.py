...
Description:
  Ensure your Amazon Machine Images (AMIs) are not older than given number of days.
  The rule is NON_COMPLIANT if one or more Amazon Machine Images are older than given number of Days.
Trigger:
  Periodic
Reports on:
  AWS::EC2::Instance
...
Scenarios:
  Scenario: 1
    Given: No AMIs older given number of days
     Then: Return COMPLIANT
  Scenario: 2
    Given: One or more AMIs older than given number of Days
...
DEFAULT_RESOURCE_TYPE = 'AWS::EC2::Image'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    images = get_image_usage(valid_rule_parameters)
    if images:
        for image in images:
            if (datetime.datetime.utcnow() - datetime.datetime.strptime(image["CreationDate"], "%Y-%m-%dT%H:%M:%S.%fZ")).days > int(valid_rule_parameters["numberOfDays"]):
                evaluations.append(build_evaluation(
                    image['ImageId'], 'NON_COMPLIANT', event))
            else:
                evaluations.append(build_evaluation(
                    image['ImageId'], 'COMPLIANT', event, annotation='The naming (tagging) structure of the specified AMIs does not adhere to AWS tagging best practices'))
    else:
        evaluations.append(build_evaluation(
            event['accountId'], 'COMPLIANT', event, annotation='The available images do not have name tags defined.'))
    return evaluations


def get_image_list(valid_rule_parameters, NextToken=None):
    if NextToken:
...
def get_image_usage(valid_rule_parameters):
    response = get_image_list(valid_rule_parameters)
    image_list = []
    # make describe_snapshots call again if the NextToken is present.
    while True:
        for image in response["Images"]:
            image_list.append(image)
        if "NextToken" in response:
            response = get_image_list(
                valid_rule_parameters, NextToken=response['NextToken'])
        else:
            break
    return image_list
...
def build_parameters_value_error_response(ex):
    """Return an error dictionary when the evaluate_parameters()
    raises a ValueError.
    ...
    return build_error_response(
        ...
        internal_error_details="An ValueError was raised during the \
...
def get_client(service, event):
    """Return the service boto client. It should be used instead of
     directly calling the client.
...
def build_evaluation(resource_id, compliance_type, event,
    ...
    eval_cc['OrderingTimestamp'] = str(json.loads(
        event['invokingEvent'])['notificationCreationTime'])
    return eval_cc


def build_evaluation_from_config_item(configuration_item,
                                      compliance_type, annotation=None):
    """Form an evaluation as a dictionary. Usually suited to report
     on configuration change rules.
...
# Check whether the message is OversizedConfigurationItem
# ChangeNotification or not
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        ...
        for evaluation in compliance_result:
            ...
            for field in ('ComplianceResourceType',
...
def build_internal_error_response(internal_error_message,
                                  internal_error_details=None):
    return build_error_response(internal_error_message,
                                internal_error_details,
                                'InternalError', 'InternalError')


def build_error_response(internal_error_message,
                         internal_error_details=None,
