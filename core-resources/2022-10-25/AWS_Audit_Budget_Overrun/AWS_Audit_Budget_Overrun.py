...
Description:
    The rule is NON_COMPLIANT if the Actual spend amount is more than Budget Limit Amount
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for budget in budget_details:
        if float(budget['CalculatedSpendAmount']) <= float(budget['BudgetLimitAmount']):
            evaluations.append(build_evaluation(budget['budgetname'], 'COMPLIANT', event))
        else:
...
def get_budget_details(event):
    ...
    for budget in response['Budgets']:
        budget_dict = dict()
        budget_dict['CalculatedSpendAmount'] = budget['CalculatedSpend']['ActualSpend']['Amount']
