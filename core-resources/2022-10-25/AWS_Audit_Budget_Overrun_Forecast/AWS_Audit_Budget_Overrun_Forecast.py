...
Description:
    The rule is COMPLIANT if the Budgets are described
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    budget_details = get_budget_details(event)
    print("budget_details", budget_details)
    # No Budget exists
    if not budget_details:
        return None

    for budget in budget_details:
        if float(budget['CalculatedSpendAmount']) <= float(budget['BudgetLimitAmount']):
            evaluations.append(build_evaluation(budget['budgetname'], 'NON_COMPLIANT', event,
                                                annotation='This Budget is more than the specified budget limit'))
        else:
            evaluations.append(build_evaluation(budget['budgetname'], 'NON_COMPLIANT', event))
    return evaluations


def get_budget_details(event):
    budget_list = list()
    response = get_budget_list(event)

    for budget in response['Budgets']:
        budget_dict = dict()
        if 'ForecastedSpend' in budget['CalculatedSpend']:
            if budget['CalculatedSpend']['ForecastedSpend']['Amount']:
                budget_dict['CalculatedSpendAmount'] = budget['CalculatedSpend']['ForecastedSpend']['Amount']
        else:
            budget_dict['CalculatedSpendAmount'] = 0
        budget_dict['BudgetLimitAmount'] = budget['BudgetLimit']['Amount']
        budget_dict['budgetname'] = budget['BudgetName']
        budget_list.append(budget_dict)
        print("budget_list", budget_list)

    # # # make describe call again if the nextToken is present.
    while True:
        if "nextToken" in response:
            response = get_budget_list(event, nextToken=response['nextToken'])
            for budget in response['Budgets']:
                budget_dict = dict()
                if 'ForecastedSpend' in budget['CalculatedSpend']:
                    if budget['CalculatedSpend']['ForecastedSpend']['Amount']:
...
def get_budget_list(event, nextToken=None):
    if nextToken:
        response = AWS_BUDGET_CLIENT.describe_budgets(AccountId=event['accountId'], MaxResults=20, nextToken=nextToken)
    else:
        response = AWS_BUDGET_CLIENT.describe_budgets(AccountId=event['accountId'], MaxResults=20)
    return response


def evaluate_parameters(rule_parameters):
    try:
        if "region_id" in rule_parameters and rule_parameters["region_id"] != "":
            valid_rule_parameters = rule_parameters
        else:
            print("Please specify a valid region_id")
        return valid_rule_parameters
    except LookupError:
        print("Please input region_id as the key.")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'ruleParameters' in event:
            rule_params = json.loads(event['ruleParameters'])
            region_id = rule_params.get("region_id")
        AWS_CONFIG_CLIENT = get_client('config', event, region_id)
        AWS_BUDGET_CLIENT = get_client('budgets', event, region_id)
