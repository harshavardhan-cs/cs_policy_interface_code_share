...
def evaluate_compliance(event):
    ...
    client = get_client('ecs', event)
    evaluations = []
    response = client.list_account_settings(name="containerInsights", effectiveSettings=True)
    data = response["settings"]
    if data[0]["value"] == "disabled":
        evaluations.append(build_evaluation(data[0]["name"], 'NON_COMPLIANT', event))
    else:
        evaluations.append(build_evaluation(data[0]["name"], 'COMPLIANT', event))
    return evaluations
