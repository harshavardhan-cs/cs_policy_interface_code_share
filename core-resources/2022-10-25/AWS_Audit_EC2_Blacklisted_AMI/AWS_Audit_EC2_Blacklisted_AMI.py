'''
Rule Name:
  AWS_Audit_EC2_Blacklisted_AMI
Description:
  Ensure there are no AWS EC2 instances launched from blacklisted AMIs.
  The rule is NON_COMPLIANT if one or more Amazon Machine Images ID .
Trigger:
  Periodic
Reports on:
  AWS::::Account
Rule Parameters:
  ...
  | ---------------------- | ---------- | -------------------------------------------------------------- |
  | Blacklisted_AMIs       | Mandatory  | List of AMIs which are blacklisted                             |
  | ---------------------- | ---------- | -------------------------------------------------------------- |
Scenarios:
  Scenario: 1
    Given: No AMIs from "Blacklisted AMI" rule parameter is found
     Then: Return COMPLIANT
  Scenario: 2
    Given: One or more AMIs from "Blacklisted AMI" rule parameter is found
     Then: Return NON_COMPLIANT with Annotation containing found AMI IDs
'''
...
# Limit the annotation string to 256 character
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    all_ami_result = ec2_client.describe_images(
        Owners=[event['accountId']]
    )
    # If all_ami_list is not empty and found blacklisted, generate non-compliant response
    if all_ami_result['Images']:
        all_ami_list = generate_image_id_list(ami_image_result['Images'], event)
        black_listed = get_blacklisted_AMI_in_all_available(all_ami_list, valid_rule_parameters):
        if black_listed:
            evaluations = []
            evaluations.append(
                build_evaluation(
                    ...
                    'NON_COMPLIANT',
                    event,
                    annotation='Found Blacklisted Amazon Machine Image Id: {}'.format(black_listed)
                )
            )
            return evaluations
...
def evaluate_parameters(rule_parameters):
    ...
    if 'Blacklisted_AMIs' not in rule_parameters:
        raise ValueError('The parameter "Blacklisted_AMIs" must be configured.')

    param_list = rule_parameters['Blacklisted_AMIs']
    param_list = param_list.split(',')

    for index, item in enumerate(param_list):
        param_list[index] = item.strip()

    valid_rule_parameters['Blacklisted_AMIs'] = param_list
    return valid_rule_parameters


def get_blacklisted_AMI_in_all_available(all_ami_result, valid_rule_parameters)
    found_blacklisted_AMI = []
    blacklisted = valid_rule_parameters['Blacklisted_AMIs']
    for ami in all_ami_result:
        if ami in blacklisted:
            found_blacklisted_AMI.append(ami)

    return found_blacklisted_AMI
...
def is_applicable(configuration_item, event):
    ...
    if status == 'ResourceDeleted':
        LOG.info("Resource Deleted, setting Compliance Status to NOT_APPLICABLE.")
