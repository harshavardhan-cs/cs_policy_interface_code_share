'''
Rule Name:
  AWS_Audit_EC2_Publicly_Shared_AMI
Description:
  Ensure your Amazon Machine Images (AMIs) are not accessible to all AWS accounts.
  The rule is NON_COMPLIANT if one or more Amazon Machine Images are publicly accessible.
Trigger:
  Periodic
Reports on:
  AWS::::Account
    ...
    if public_ami_result['Images']:
        ...
        evaluations.append(
            build_evaluation(
                ...
                'NON_COMPLIANT',
                event,
                annotation='Public Amazon Machine Image Id: {}'.format(
                    ",".join([generate_image_id_list(public_ami_result['Images'], event)]))
            )
        )
        return evaluations
