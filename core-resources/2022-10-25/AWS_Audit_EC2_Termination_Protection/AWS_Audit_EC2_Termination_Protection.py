...
###
 ...
 Rule Name:
   AWS_Audit_EC2_Termination_Protection
 Description:
   Check whether EC2 Termination Protection is enabled.

 Rationale:
   Regular checks to ensure that EC2 Termination Protection is enabled.

 Indicative Severity:
   High

 Trigger:
   Periodic checks on AWS::EC2::Instance
 ...
 Scenarios:
  Scenario: 1
    Given: EC2 Instance has termination protection enabled 
     Then: Return COMPLIANT
  Scenario: 2
    Given: EC2 Instance has termination protection disabled 
...
def evaluate_compliance(event):
    ...
    instancesinfo = ec2.describe_instances()

    for instanceinfo in instancesinfo['Reservations']:
        for instance in instanceinfo['Instances']:
            # print (instance['InstanceId'])
            instance_attribute = ec2.describe_instance_attribute(InstanceId=instance['InstanceId'],
                                                                 Attribute='disableApiTermination')
            # print (instance_attribute['DisableApiTermination'])
            if instance_attribute['DisableApiTermination']['Value'] == False:
                # print('Instance Not Secured: ' + instance['InstanceId'])
                evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event,
                                                    annotation='EC2 Termination Protection is disabled'))
            else:
                evaluations.append(build_evaluation(event["accountId"], 'COMPLIANT', event,
                                                    annotation='EC2 Termination Protection is enabled'))

    return evaluations


def get_groups_and_roles_policies(client, group_response, role_response):
    group_policy_names = []
    roles_policy_names = []
    for group in group_response["Groups"]:
        if group:
            list_group_policies = client.list_attached_group_policies(GroupName=group["GroupName"])
            for policy in list_group_policies["AttachedPolicies"]:
                group_policy_names.append(policy["PolicyName"])
    for role in role_response["Roles"]:
        if role:
            list_role_policies = client.list_attached_role_policies(RoleName=role["RoleName"])
            for policy in list_role_policies["AttachedPolicies"]:
                roles_policy_names.append(policy["PolicyName"])
    return group_policy_names, roles_policy_names
