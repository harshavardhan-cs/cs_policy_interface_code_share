'''
Rule Name:
  ELBs listeners are using a secure protocol (HTTPS or SSL)
Description:
  Ensure that your AWS ELBs listeners are using a secure protocol (HTTPS or SSL).
  The rule is NON_COMPLIANT if one or more ELBs are using un-secure protocol.
Trigger:
  Periodic
Reports on:
...
Scenarios:
  Scenario: 1
    Given: No ELBs listeners are using HTTP or other unsecure protocol.
     Then: Return COMPLIANT
  Scenario: 2
    Given: One or more ELBs with unsecure protocol like 'HTTP', etc.
     Then: Return NON_COMPLIANT with Annotation containing ELB Names
'''
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    elb_client = get_client('elb', event)
    elb_list = elb_client.describe_load_balancers()

    LIST_HTTPS_SSL = ['https', 'ssl']
    LIST_HTTP = ['http']

    if elb_list['LoadBalancerDescriptions']:
        elb_name_list = []
        evaluations = []
        for elb in elb_list['LoadBalancerDescriptions']:
            #            print(elb)
            if elb['ListenerDescriptions']:
                for listner in elb['ListenerDescriptions']:
                    #                    print(listner)
                    try:
                        protocol = listner['Listener']['Protocol'].lower()
                        if (protocol in LIST_HTTP or
                                protocol not in LIST_HTTPS_SSL):
                            elb_name_list.append(elb['LoadBalancerName'])
                    except ValueError:
                        pass
        ...
        if elb_name_list:
            evaluations.append(
                build_evaluation(
                    ...
                    'NON_COMPLIANT',
                    event,
                    annotation='Found ELB Listener with un-secure HTTP Protocol: {}'.format(elb_name_list)
                )
            )
            return evaluations
