...
def evaluate_compliance(event):
    ...
    if response["SummaryMap"]["AccountMFAEnabled"] == 0:
        evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event,
                                            annotation='MFA is not enabled for this account'))
    else:
        res = iam_client.list_virtual_mfa_devices()
        if len(res["VirtualMFADevices"]) != 0:
            for device in res["VirtualMFADevices"]:
                if "SerialNumber" in device.keys():
                    evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event,
                                                        annotation='Root account is not protected with Hardware MFA'))
                    break
        else:
            evaluations.append(build_evaluation(event["accountId"], 'COMPLIANT', event,
                                                annotation='Root account is protected with MFA'))
