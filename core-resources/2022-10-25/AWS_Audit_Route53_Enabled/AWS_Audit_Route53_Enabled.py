...
def evaluate_compliance(configuration_item, rule_parameters):
    ...
    if not configuration_item.get('configuration', {}).get('vpcId'):
        return 'NOT_APPLICABLE'
    response = client.describe_vpc_attribute(Attribute='enableDnsSupport', VpcId=configuration_item.get('configuration', {}).get('vpcId', 'NA'))
    if response.get('EnableDnsSupport', {}).get('Value'):
        return 'COMPLIANT'
    else:
...
def lambda_handler(event, context):
    ...
    if isinstance(compliance_result, str):
        evaluations = [{
            'ComplianceResourceType': configuration_item.get('resourceType', 'AWS::::Account'),
            'ComplianceResourceId': configuration_item.get('resourceId', 'NA'),
