...
def evaluate_compliance(event):
    ...
    group_policies, roles_policies = get_groups_and_roles_policies(iam_client, groups, roles)
    if "AWSSupportAccess" not in group_policies or "AWSSupportAccess" not in roles_policies:
        evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event,
                                            annotation='AWSSupportAccess is not assigned to any of groups or roles'))
    else:
        evaluations.append(build_evaluation(event["accountId"], 'COMPLIANT', event,
                                            annotation='AWSSupportAccess policy is assigned to a group or a role in this account'))
    return evaluations
