...
def evaluate_compliance(event):
    ...
    for user_report in data[1:]:
        value = user_report.split(",")
        user = value[0]
        access_key_1_last_used = value[10]
        access_key_2_last_used = value[15]
        if access_key_1_last_used == 'N/A' and access_key_2_last_used == 'N/A':
            evaluations.append(build_evaluation(user, 'NON_COMPLIANT', event,
                                                annotation='Both the access keys last used is N/A'))
        elif access_key_1_last_used == 'N/A' or access_key_2_last_used == 'N/A':
            if access_key_1_last_used == 'N/A':
                evaluations.append(build_evaluation(user, 'NON_COMPLIANT', event,
                                                    annotation='Access key 1 last used is N/A'))
            else:
                evaluations.append(build_evaluation(user, 'NON_COMPLIANT', event,
                                                    annotation='Access key 2 last used is N/A'))
        else:
            evaluations.append(build_evaluation(user, 'COMPLIANT', event,
                                                annotation='Access keys are used'))
