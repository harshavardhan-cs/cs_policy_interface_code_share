...
def evaluate_compliance(event):
    ...
    for items in response["Reservations"]:
        name = items["Instances"][0]["PublicDnsName"]
        namelist.append(name)
    inspector_client = get_client('inspector', event)
    try:
        ...
        if 'AccessDeniedException' in str(e):
            ...
            raise e
    result_list = []
    for arn in response.get("assessmentTargetArns", []):
        result = inspector_client.preview_agents(
            previewAgentsArn=arn)
        if result['agentPreviews'][0]['agentHealth'] == "HEALTHY":
            result_list.append(result['agentPreviews'][0]["hostname"])
    data = list(set(namelist) - set(result_list))
    if data:
        for item in data:
            evaluations.append(build_evaluation(item, 'NON_COMPLIANT', event))
    else:
...
def build_evaluation_from_config_item(configuration_item, compliance_type, annotation=None):
    ...
    eval_ci['ComplianceResourceId'] = configuration_item.get('resourceId', 'NA')
...
def clean_up_old_evaluations(latest_evaluations, event):
    ...
    for old_eval in old_eval_list:
        ...
        for latest_eval in latest_evaluations:
            if old_resource_id == latest_eval.get('ComplianceResourceId', 'NA'):
                newer_founded = True
            if (datetime.datetime.now() - start_time).total_seconds() >= 450:
        ...
        for evaluation in compliance_result:
            ...
            for field in ('ComplianceResourceType', 'ComplianceResourceId', 'ComplianceType', 'OrderingTimestamp'):
                if field not in evaluation:
                    ...
                    missing_fields = True
                if (datetime.datetime.now() - start_time).total_seconds() >= 750:
                    break
