...
def evaluate_compliance(event, client):
    ...
    for recorder in response["ConfigurationRecorders"]:
        if recorder["recordingGroup"]["allSupported"] and recorder["recordingGroup"]["includeGlobalResourceTypes"]:
            config_names.append(recorder["name"])

    if config_names:
        recorder_status = client.describe_configuration_recorder_status(ConfigurationRecorderNames=config_names)
    ...
    try:
        ...
        if invoking_event['messageType'] in ['ConfigurationItemChangeNotification', 'ScheduledNotification',
            ...
            if is_applicable(configuration_item, event):
                compliance_result = evaluate_compliance(event, AWS_CONFIG_CLIENT)
            else:
