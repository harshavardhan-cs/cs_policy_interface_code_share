...
def evaluate_compliance(event, params):
    LOG.info("Compliance Evaluation Started")
    ec2_client = get_client('ec2', event)
    evaluations = []
    response = ec2_client.describe_route_tables()
    LOG.info(params["address_space"])
    for route_table in response["RouteTables"]:
        for route in route_table["Routes"]:
            if route["GatewayId"].split("-")[0] == "pcx":
                if route["DestinationCidrBlock"] != str(params["address_space"]):
                    evaluations.append(build_evaluation(route_table["RouteTableId"], 'NON_COMPLIANT', event,
                                                        annotation=''))
...
def evaluate_parameters(rule_parameters):
    ip_address_pattern = '(([2][5][0-5]\.)|([2][0-4][0-9]\.)|([0-1]?[0-9]?[0-9]\.)){3}(([2][5][0-5])|([2][0-4][0-9])|([0-1]?[0-9]?[0-9]))'
    range = '^[0-9]{1,2}$'
    ip_parts = rule_parameters["address_space"].split('/')
    ip_address_check = re.match(ip_address_pattern, ip_parts[0])
    range_check = re.match(address_range, ip_parts[1])
    if ip_address_check and range_check:
        valid_rule_parameters = rule_parameters
    else:
        return build_parameters_value_error_response("Ip range is invalid")
...
def lambda_handler(event, context):
    ...
    if 'ruleParameters' in event:
        rule_parameters = ast.literal_eval(event['ruleParameters'])
    ...
    try:
        ...
        if invoking_event['messageType'] in ['ConfigurationItemChangeNotification', 'ScheduledNotification',
            ...
            if is_applicable(configuration_item, event):
                LOG.info(valid_rule_parameters)
