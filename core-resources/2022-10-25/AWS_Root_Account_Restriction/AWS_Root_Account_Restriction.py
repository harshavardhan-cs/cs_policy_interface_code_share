...
def evaluate_compliance(event):
    ...
    report_encoded = response['Content']
    root = list()
    check_datelist = list()
    for data in report_encoded.decode('utf-8').split('\n'):
        if data.__contains__('root_account'):
            root = data.split(',')
    root_access_last_used = root[10].split('T')[0]
    current_date = str(datetime.datetime.utcnow()).split(" ")[0]
    today = datetime.datetime.strptime(current_date, '%Y-%m-%d')
    before = datetime.datetime.strptime(current_date, '%Y-%m-%d') - datetime.timedelta(days=30)
    time_data = (before + datetime.timedelta(days=x) for x in range(0, (today - before).days + 1))
    for date in time_data:
        check_datelist.append(date.strftime("%Y-%m-%d"))
    if root_access_last_used in check_datelist:
        evaluations.append(build_evaluation(event["accountId"], 'NON_COMPLIANT', event,
                                            annotation='The root account has been used in past month'))
    else:
        evaluations.append(build_evaluation(event["accountId"], 'COMPLIANT', event,
                                            annotation='The root account has not been used'))
    return evaluations
...
def clean_up_old_evaluations(latest_evaluations, event):
    ...
    for old_eval in old_eval_list:
        ...
        if (datetime.datetime.now() - start_time).total_seconds() >= 400:
